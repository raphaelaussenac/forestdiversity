TestPlotIndex <- function(){
    ...
    Compute_mingling(Td)
    Compute_mingling(Td2)
    Compute_mingling(Td3)
    ...
    DFgrid3 <- dplyr::mutate(DFgrid, D_cm=rep(seq(10,60,length.out=2),
	 ((N+2)^2)/2)[1:(N+1)^2])
    Td3 <- TabDist(DFgrid3, coord=c(0, N, 0, N))
    Compute_Size_Diff(Td)
    Compute_Size_Diff(Td2)
    Compute_Size_Diff(Td3)
    ...
    Td3 <- TabDist(DFgrid3, coord=c(0, N, 0, N))
    Compute_Winkelmass(Td)
    Compute_Winkelmass(Td2)
    Compute_Winkelmass(Td3)
}
