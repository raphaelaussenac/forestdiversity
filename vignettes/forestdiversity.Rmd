...
These 4 indices, are computed based on abundances.
If we $p_i$ the proportion of each class and N the total number of classes:

1. $Nclass = N$ (=$^qD$ with $q = 0$)

2. $Shannon = \sum_{i=1}^{i=N}( - p_i * log(p_i))$ (=log($^qD$) with q ->1)

3. $Simpson = \sum_{i=1}^{i=N}(p_i^2)$ (=$\dfrac{1}{^qD}$ with q = 2)

4. $GiniSimpson = 1 - Simpson$ 

$^qD$ being the Hill number.
