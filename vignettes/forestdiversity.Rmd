...
```{r echo=TRUE}
DF <- read.csv('../data/virtualExperiment.csv')
DF <- format_Salem(DF)
print(head(DF))
```

We use EventResilience function to compute metrics:


Some notation explanation: Theta is the recovery rate, TimeRec the time to recovery and DegRec the degree of recovery after X years (here 20).
