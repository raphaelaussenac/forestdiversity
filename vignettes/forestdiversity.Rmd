...
# Diversity/heterogeneity indices
...
We have a dataset named named PlotExample with at least trees species, DBH, and location in X / Y:
...
```{r echo=TRUE}
E <- EventResilience(DF, Nvar='V_m3', RecTime=20, normalize='baseline')
print(head(E))
```
...
The function ComputeSh will compute different heterogeneity indices from the distriution dataset (here LandscapeExample).
...
5. The variance of the mean quadratic diameter in each grid point

For example to compute heterogeneity metrics at 0.1 km resolution :
```{r echo=TRUE}
ComputeShScale(LandscapeExample, Res=0.1)
```
All metrics are computed using 9 grid configuration (variable N in the data.frame).

```{r echo=TRUE}
plotHeterogeneityScale(LandscapeExample, Res=c(0.05, 0.1, 0.5), PLOT=TRUE)
```
